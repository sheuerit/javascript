// 객제(Object)
// - 연관된 데이터를 담는 그릇
// - 객체의 내용물이 바뀌어도 본질은 같은 객체
// - 객체는 키(key)와 값(value)로 구성
// - 객체가 가지고 있는 값을 속성(프로퍼티), 함수인 경우 메서드
// - 속성(프로퍼티)은 기본 타입이 될 수도 있고, 메서드를 가질 수도 있고, 배열과 객체를 프로퍼티로 가질 수 있다
const obj = {
    // name = 키(key)
    // "송현석" = 값(value)
    name: "송현석", // 속성(프로퍼티)
    age: 50,
    hello: function() { // 메서드
        console.log("hello");
    },
    hobby: ["운동", "게임", "독서"],
    score: {
        "수학": 100,
        "영어": 90
    },
    parent: parent
}
const parent = {
    name: "홍길동"
};

// 객체 생성
// - 리터럴 방식: 중괄호{} 안에 key : value 형태를 쉼표로 구분하여 생성
// - 생성자 방식: new 연산자와 Object 생성자를 사용하여 생성

// 리터럴 방식
// const object = {
//     name: '송현석',
//     age: 99
// };
const object1 = {};
object1.name = '송현석';
object1.age = 99;

// 생성자 방식
const object2 = new Object();
object2.name = '송현석';
object2.age = 99;

console.log(object2);

// 객체 프로퍼티 접근
// - 점표기법, 괄호 표기법
const person = {
    name: '홍길동',
    age: 99,
    hobby: ["운동", "게임", "독서"],
    hello: function() { // 메서드
        console.log("hello");
    },
    children: {
        name: '송현석'
    }    
};
// 점 표기법
person.name;
person.age;
person.hobby[1];
person.hello();
person.children.name;

console.log(person.children.name);

// 괄호 표기법
person['name'];
person['age'];
person['hobby'][1];
person['hello']();
person['children']['name'];

console.log(person['hobby'][1]);

// 객체 프로퍼티 생성 및 제거
// 생성
object1.name = "송현석";
object1.age = 99;

// 제거
delete object1.age;

// 함수 파라미터에 객체 전달
function printPerson(name, age, hobby) {
    console.log('이름: ' + name + '나이: ' + age + '취미: ' + hobby);
}
printPerson("전현우", 100, "게임");

// 객체로 전달
function printPerson2() {
    console.log('이름: ' + name + '나이: ' + age + '취미: ' + hobby);
}
const objPerson = {
    name: '송현석',
    age: 90,
    hobby: '운동'
};
printPerson2(objPerson);

// 구조 분해 할당
// - 배열이나 객체의 속성을 개별 변수에 담을 수 있게하는 표현식
const temp = {
    name: "송현석",
    age: 99,
    job: "개발자"
};
// const name = temp.name;
// const age = temp.age;
// const job = temp.job;

// 구조 분해 할당을 사용하여 개별 변수에 담을 때
const {name, age, job} = temp;

// 구조 분해 할당을 사용하여 함수에서 객체 개별 속성을 받을 때
function tempPerson ({name, age, job}){
    console.log('이름: ' + name + '나이: ' + age + '직업: ' + job);
}
tempPerson(temp);

// 팩토리 함수
// - 객체를 반환하는 함수
// - 같은 유형의 객체를 여러번 생성하는 경우 사용
// - 같은 유형의 객체를 여러번 생성하면 번거롭고 가독성이 떨어지는데 팩토리 함수로 해결

// 같은 유형의 객체 여러번 생성
const person1 = {
    name: '홍길동',
    age: 10,
    hobby: '운동'
};
const person2 = {
    name: '고길동',
    age: 20,
    hobby: '게임'
};
const person3 = {
    name: '김길동',
    age: 30,
    hobby: '독서'
};

// 팩토리 함수 사용
function createPerson(name, age, hobby) {
    return {
        name: name,
        // age: age,
        age,
        // hobby: hobby
        hobby // 단축 속성명은 ES6에서 추가
    };
};
const person4 = createPerson('홍길동', 10, '운동');
const person5 = createPerson('고길동', 20, '게임');
const person6 = createPerson('김길동', 30, '독서');

// 참조 타입(객체 타입)
// 기본 타입(원시 타입, 프리미티브 타입) 복사
// 기본 타입은 복사할 때 값 자체를 넘겨주기 때문에 값의 원형이 바뀌지 않음
let str1 = 'hello';
let str2 = str1;
str2 = "world";
console.log(str1);
console.log(str2);

// 참조 타입(객체 타입) 복사
// 객체를 다른 변수에 할당하면 참조값이 할당 되기 때문에 할당 받은 변수에서 속성을 변경하면 값의 원형이 변경
let obj1 = {name: '송현석', age: 20};
let obj2 = obj1;
obj2.age = 30;
console.log(obj1);
console.log(obj2);

// Call by Value : 기본 타입 호출
function change(username) {
    username = '홍길동';
}
let callName = '송현석';
change(callName);
console.log(callName);

// Call by Reference : 참조 타입 호출
// 값을 가르키는 참조값을 people에 할당했기 때문에 people의 속성을 변경하면 callName의 속성 값도 함께 변경
function change2(people) {
    people.name = '홍길동';
}
let callName2 = { name: "송현석" };
change2(callName2);
console.log(callName2);

// 전개 구문
// - 객체 안의 프로퍼티를 펼치는 방식으로 할당
// - ... 표기법 사용
const obj3 = {
    name: '송현석',
    age: 20
};
const obj4 = {
    ...obj3,
    hobby: '운동',
};
console.log(obj4);

// 객체 복사
// - 다른 참조값을 가지는 객체를 완전히 복사
// - 전개 구문을 이용하는 복사, Object.assign()를 사용하는 복사

// 전개 구문 사용
const obj5 = {
    name: '송현석'
};
const obj6 = {...obj5};
obj6.name = '홍길동';
console.log(obj5);
console.log(obj6);

// Object.assign() 사용
const obj7 = {
    name: '송현석'
};
const obj8 = Object.assign({}, obj7);
obj8.name = "홍길동"
console.log(obj7);
console.log(obj8);

// for ... in
// - 객체의 항목들을 반복하여 반환
const obj9 = {
    name: '홍길동',
    age: 20,
    job: '개발자'
};
for(let key in obj9){
    console.log(key + ", " + obj9[key]);
}

// 키 존재 여부 확인
// - key in object, Object.hasOwn(), obj.hasOwnProperty(), if문 사용
const obj10 = {
    name: '홍길동'
};

// key in object
console.log('name' in obj10); // true & false

// Object.hasOwn()
console.log(Object.hasOwn(obj10, 'name')); // true & false

// obj.hasOwnProperty()
console.log(obj10.hasOwnProperty('name')); // true & false

// if문 사용
if (obj10.name) {
    console.log("프로퍼티가 있습니다");
} else {
    console.log("프로퍼티가 없습니다");
}